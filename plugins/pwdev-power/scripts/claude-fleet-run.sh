#!/usr/bin/env bash
# Run a fleet member's autonomous lifecycle on the claude runtime.
#
...
[[ $# -eq 2 || $# -eq 3 ]] || fail 'usage: claude-fleet-run.sh <slug> <worktree> [permission-mode]'
SLUG=$1
