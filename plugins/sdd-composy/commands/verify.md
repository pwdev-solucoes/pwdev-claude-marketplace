...
`${CLAUDE_PLUGIN_ROOT}/skills/sdd-verify/SKILL.md`, pass through `$ARGUMENTS` and the
current repository context, and return the shared skill's result unchanged.

The shared task engine owns artifact predicates and lifecycle transitions; this adapter
must not infer approval or complete a task directly.
