...
import hashlib, re, subprocess, time
...
def preflight(task, *, red_evidence=False, changed_paths=(), environment="environment_owned", human_approved=False):
    ...
    if any(d.get("state")!="complete" for d in t.get("dependencies",[])): return _base(t,"blocked","blocked","dependency_missing","complete dependencies")
    ...
    outside=[p for p in changed_paths if not any(p==a or p.startswith(a.rstrip("/")+"/") for a in t.get("allowed_paths",()))]
