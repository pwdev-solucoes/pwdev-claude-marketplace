...
description: >
  ...
  NOT use for a single task (sdd-execute, sdd-loop), for merging without the
  explicit teardown token, or on OpenCode (no fleet engine).
metadata:
  version: 0.1.0
---
...
only `ready` tasks with complete dependencies, explicit acceptance criteria, known verification
commands, and no confirmed path overlap. It never infers approval and never merges fleet branches
automatically.
...
- `launch`: validate contracts and create recoverable branches and worktrees
  (`scripts/fleet/launch.sh`). Before launch, report the selected tasks, runtime, UI adapter,
  branch and worktree paths, verification commands, and safety checks.
...
- `teardown`: stop a member, or merge a terminal member only with `--merge --confirm
  CONFIRM-SDD-MERGE` after identity and result verification (`scripts/fleet/teardown.sh`).

`scripts/fleet/run.sh` is the shared stage runner. Provider command vectors live only in the
...
- `references/fleet.md` — before `launch` or `teardown`, for record schema, Compose, migration,
  and merge rules.
