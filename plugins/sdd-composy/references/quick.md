...
Failure of any condition means `ESCALATED`; stop before editing and continue through
...
2. Write the `QUICK_CONTRACT` and register the normal task through `sdd_tasks.py`.
...
5. After each represented action succeeds, append semantic events with `sdd_trace.py`;
...
The exact confirmation token `CONFIRM-SDD-SYNC` is required only for synchronization
