...
| `review_required -> verify_required` | review has no critical, major, or otherwise blocking finding |
...
Global state records the active lifecycle stage, active PRD/task, last gate, blockers, loop/fleet summaries, trace health, update timestamp, and exact next valid action. Stage state advances only after its durable output exists and validates. Failed publication leaves the prior state authoritative.
