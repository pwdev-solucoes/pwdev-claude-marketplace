...
PROVIDER_UNAVAILABLE = re.compile(r"(?i)(service temporarily overloaded|overloaded|temporarily unavailable|service unavailable|"
                                  r"bad gateway|upstream error|\[50[234]\])[^\n]{0,160}")
...
def prepare(runtime: str, request: RunRequest) -> Dict[str, Any]:
    ...
    if runtime == "claude":
        ...
        name = request.skill_name or Path(request.skill_dir).name if request.skill_dir else "bench"
...
def run(runtime: str, request: RunRequest) -> Dict[str, Any]:
    ...
    budget_hit = (request.budget_usd is not None and isinstance(record.get("cost_usd"), (int, float))
                  and record["cost_usd"] >= request.budget_usd)
    if outcome["timed_out"]:
        record.update(status="BLOCKED", reason=f"timeout after {request.timeout:.0f}s")
    elif QUOTA_ERROR.search(combined):
        # The provider refused to serve: no evidence about the skill either way.
        record.update(status="NOT_RUN", reason="runtime quota or usage limit: " + sanitize(_first_match(QUOTA_ERROR, combined)))
    elif not parsed.get("envelope_ok") and PROVIDER_UNAVAILABLE.search(combined):
        # The provider failed to serve (5xx, overload): no evidence about the skill either way.
        record.update(status="NOT_RUN", reason="provider unavailable: " + sanitize(_first_match(PROVIDER_UNAVAILABLE, combined)))
    elif outcome["exit_code"] != 0 and budget_hit:
        # The per-run cap stopped the model mid-task: it says nothing about the skill.
        record.update(status="NOT_RUN",
                      reason=f"per-run budget of US$ {request.budget_usd:.4f} reached (spent {record['cost_usd']:.4f})")
    elif outcome["exit_code"] != 0 and MODEL_ERROR.search(combined):
        record.update(status="NOT_RUN", reason="runtime rejected the model: " + sanitize(outcome["stderr"].strip()[:300]))
    elif outcome["exit_code"] != 0:
        record.update(status="FAIL", reason=f"exit {outcome['exit_code']}: " + sanitize(outcome["stderr"].strip()[:300]))
    else:
        record.update(status="PASS" if parsed["envelope_ok"] else "FAIL",
                      reason=None if parsed["envelope_ok"] else "runtime envelope missing or reported an error")
    if record["protected_changed"]:
        record.update(status="FAIL", reason="a protected path changed during the run")
