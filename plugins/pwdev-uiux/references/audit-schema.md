...
in the project. Rows are distinguished by the `plugin` column (`pwdev-uiux` vs
`pwdev-code`). Either plugin's `init` may create the schema — it is identical.
Query this plugin's rows with `WHERE plugin='pwdev-uiux'`. Note: an
`agent_type` without a plugin namespace could be logged by both plugins'
hooks; spawns always use `pwdev-uiux:executor`, so this is rare.
...
## Action vocabulary (subset used by pwdev-uiux)
...
WHERE plugin='pwdev-uiux' ORDER BY id DESC LIMIT 50;
-- Executor average duration
SELECT AVG(duration_ms) avg_ms FROM events
WHERE plugin='pwdev-uiux' AND action='completed' AND duration_ms IS NOT NULL;
