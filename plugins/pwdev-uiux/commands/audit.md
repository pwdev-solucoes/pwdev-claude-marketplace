...
- **Plugin hooks** (`hooks/hooks.json` → `scripts/audit-hook.sh`): session
  start/stop, subagent runs with real `duration_ms`/`session_id`, and
...
The database is SHARED with the other PWDEV plugins — filter this plugin's
rows with `WHERE plugin='pwdev-uiux'`.
