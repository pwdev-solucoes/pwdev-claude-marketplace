...
# /pwdev-uiux:audit — Audit Trail Query
...
1. Read `.planning/config.json` — if `audit` is not `true`, stop with:
   - PT-BR: `Trilha de auditoria nao esta ativada. Execute /pwdev-uiux:init para habilitar.`
   - EN: `Audit trail is not enabled. Run /pwdev-uiux:init to enable it.`
...
Examples:
  /pwdev-uiux:audit
  /pwdev-uiux:audit events
  /pwdev-uiux:audit decisions
  /pwdev-uiux:audit stats
  /pwdev-uiux:audit export
...
- PT-BR: `Mostrando os 50 eventos mais recentes. Use /pwdev-uiux:audit query para consultas customizadas.`
- EN: `Showing the 50 most recent events. Use /pwdev-uiux:audit query for custom queries.`
...
*Report generated by /pwdev-uiux:audit export*
```
...
```bash
[ -f ".planning/pwdev-audit.db" ] && sqlite3 .planning/pwdev-audit.db "INSERT INTO events (plugin, command, action, detail) VALUES ('pwdev-uiux', 'audit', 'completed', '{\"sub_command\": \"$SUB_COMMAND\"}');" 2>/dev/null
```
