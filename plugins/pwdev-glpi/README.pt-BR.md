...
MCP próprio (stdio, iniciado via `npx`) que expõe 16 tools, 2 prompts e
...
| Peça | Função |
|---|---|
| MCP `glpi` | `npx -y @soarescbm/mcp-glpi@0.1.0` — CRUD de tickets + followups + solução/fechamento, leitura de usuários, grupos, ativos (Computer/Monitor/Phone/NetworkEquipment), projetos e base de conhecimento |
...
A versão do npm é **pinada** (`@0.1.0`) por reprodutibilidade e cache do npx;
