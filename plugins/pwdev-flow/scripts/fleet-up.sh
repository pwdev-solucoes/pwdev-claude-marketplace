...
SLUG=${1:-}
[[ $# -eq 1 ]] || usage
[[ $SLUG =~ ^[a-z0-9][a-z0-9-]*$ && $SLUG == *[a-z]* && $SLUG != dashboard ]] || fail_preflight "invalid slug: $SLUG"
...
# Runtime files that must never reach the fleet branch, because fleet-run.sh
# stages every stage with `git add -A` and teardown --merge lands that history on
# the user's base branch. fleet-logs holds raw provider stdout and stderr.
# One definition, used by both the DRY_RUN plan and the live publication.
printf -v IGNORE_CONTENTS '\n# Flow fleet runtime files\n.env.fleet\n%s\n.planning/flow/fleet-status.json\n.planning/flow/fleet-logs/\n.planning/flow/fleet-results/\n' "$COMPOSE_FILE"
...
printf '%s' "$IGNORE_CONTENTS" >>"$IGNORE_TEMP" || recover 2 'could not update .gitignore'
