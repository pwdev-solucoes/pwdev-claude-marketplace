...
# /pwdev-feat:audit — Audit Trail Query
...
1. Read `.planning/config.json` — if `audit` is not `true`, stop with:
   - PT-BR: `Trilha de auditoria nao esta ativada. Execute /pwdev-feat:init para habilitar.`
   - EN: `Audit trail is not enabled. Run /pwdev-feat:init to enable it.`
...
Examples:
  /pwdev-feat:audit
  /pwdev-feat:audit events
  /pwdev-feat:audit decisions
  /pwdev-feat:audit stats
  /pwdev-feat:audit export
...
- PT-BR: `Mostrando os 50 eventos mais recentes. Use /pwdev-feat:audit query para consultas customizadas.`
- EN: `Showing the 50 most recent events. Use /pwdev-feat:audit query for custom queries.`
...
*Report generated by /pwdev-feat:audit export*
```
...
```bash
[ -f ".planning/pwdev-audit.db" ] && sqlite3 .planning/pwdev-audit.db "INSERT INTO events (plugin, command, action, detail) VALUES ('pwdev-feat', 'audit', 'completed', '{\"sub_command\": \"$SUB_COMMAND\"}');" 2>/dev/null
```
