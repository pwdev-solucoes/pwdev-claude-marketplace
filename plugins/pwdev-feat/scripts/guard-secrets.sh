...
# Deterministic enforcement of the pwdev-feat security rule: never read
...
if printf '%s' "$TARGET" | grep -Eq '(^|[/ "'"'"'])\.env([^a-zA-Z]|\.local|\.production|\.development|\.staging|$)|\.pem([^a-zA-Z]|$)|\.key([^a-zA-Z]|$)|id_rsa'; then
  echo "pwdev-feat guard: blocked access to secret file ($TARGET). Use .env.example instead." >&2
  exit 2
fi
