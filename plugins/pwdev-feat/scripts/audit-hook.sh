#!/bin/sh
# audit-hook.sh <event> — pwdev-feat deterministic audit trail.
...
case "$EVENT" in
  session_start)
    sqlite3 "$DB" "INSERT INTO events (session_id, plugin, command, action) \
      VALUES ('$(esc "$SESSION")', 'pwdev-feat', 'session', 'session_start');" 2>/dev/null
    ;;
  ...
  subagent_start)
    ...
    sqlite3 "$DB" "INSERT INTO events (session_id, plugin, command, agent, action) \
      VALUES ('$(esc "$SESSION")', 'pwdev-feat', 'subagent', '$(esc "$AGENT")', 'started');" 2>/dev/null
    ;;
  ...
  subagent_stop)
    ...
    sqlite3 "$DB" "INSERT INTO events (session_id, plugin, command, agent, action, duration_ms) \
      VALUES ('$(esc "$SESSION")', 'pwdev-feat', 'subagent', '$(esc "$AGENT")', 'completed', ${DUR:-NULL});" 2>/dev/null
    ;;
  ...
  stop)
    sqlite3 "$DB" "INSERT INTO events (session_id, plugin, command, action) \
      VALUES ('$(esc "$SESSION")', 'pwdev-feat', 'session', 'turn_completed');" 2>/dev/null
    ;;
esac
