...
Whole-branch review: quatro Majors corrigidos em ecaf57d..b584569; FR-03 de precedência por critério corrigido adicionalmente em e048b29; revisão final sem Major aberto.
Power verification: REJECTED em `.planning/power/features/pwdev-qa/verdict.md`; aceitação consolidada BLOCKED somente em CA-003, 0/3 runtimes VERIFIED. `power-finish` não executado.
Deferred Minor: fixture de par ID/texto altera o contrato sem recalcular hash e não isola o diagnóstico de par ausente; comportamento de produção foi confirmado por probe com hash correto.
Deferred Minor: exemplo de quarentena flaky ainda omite rationale e expiração delimitada; regra normativa permanece correta e o cenário permanece BLOCKED.
...
Ruling: o contrato de aceite referenciado deve ser aberto de forma confinada/no-follow, ter SHA-256 recalculado e conter cada par exato ID/texto antes da consolidação. Se estiver errado, contratos com formatação incomum poderão exigir adaptação explícita, mas relatórios não atestarão uma fonte ausente ou trocada.
Ruling: a política limitada de credenciais conhecidas aplica-se recursivamente a todas as strings da projeção pública, não apenas aos anexos; detecção recusa a publicação sem ecoar o conteúdo. Se estiver errado, algum texto sintético legítimo poderá exigir revisão manual, mas segredos conhecidos não serão propagados ao JSON/HTML/PDF.
Ruling: critério aplicável e caso obrigatório executado exigem expected/observed substantivos para PASS, mas falha comprovada vigente tem precedência e preserva FAIL; NOT_RUN/NOT_APPLICABLE continuam aceitando campos vazios. Se estiver errado, avaliações positivas precisarão de mais texto, mas a matriz não fabricará PASS nem esconderá FAIL.
Ruling: `requirements.txt` contém apenas ReportLab compatível com Python 3.9; pypdf/pdfplumber ficam em `requirements-dev.txt`. Se estiver errado, consumidores de runtime precisarão instalar leitores explicitamente para inspeção, mas a instalação suportada não exigirá Python 3.10 por dependência de desenvolvimento.
Ruling: CA-003 só passa quando cada runtime completa descoberta, invocação da skill e gera/inspeciona o relatório na mesma sessão real; descoberta estrutural ou doctor não substituem invocação. Se estiver errado, a v1 ficará bloqueada de forma conservadora em ambientes sem credenciais ou rota isolada, mas portabilidade não será alegada sem evidência.
Ruling: `.planning/power/state.md` não foi atualizado porque registra execução ativa e não relacionada de `sdd-composy`; sobrescrevê-lo destruiria verdade operacional alheia. Se estiver errado, o gate de pwdev-qa ficará apenas no ledger/verdict até reconciliação humana, mas nenhuma feature concorrente perderá seu estado.
