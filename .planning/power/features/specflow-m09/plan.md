...
# SpecFlow M09 — Network Live opcional e consentimento — Plan
...
## Goal
Network Live opcional e consentimento com critérios verificáveis e handoff explícito.

## Architecture
Recursos estáticos em plugins/sdd-flow; contratos normativos tipados na spec, qualificação nativa antes do binding. Dependência: M08 aceito, M01 central PASS e gates frescos do presente plano. Sem paralelismo. Alteração de arquitetura ou escopo retorna ao design.
...
- `tests/test_sdd_flow_m09_network.py`
- `plugins/sdd-flow/skills/sdd-flow-network/SKILL.md`
...
## Module acceptance
Local explícito não herda Live de workspace. Testar Live ausente, opt-in, limites, participação de child autorizada e não autorizada, retomada sem novo consentimento fora do escopo, nova Run exigindo escolha própria. Nenhuma mensagem concede gate ou escrita. Não ativar defaults globais. Template de skill só controla participação pelos mecanismos do runtime demonstrados em M01.
Comando focado: `python3 -m unittest tests.test_sdd_flow_m09_network`. Mais de zero testes; testes de texto/schema não substituem prova real exigida. Cada relatório registra revisão, comando, exit code, resultado, artefato e hash. Módulo rejeitado invalida dependentes afetados.

## Task 01 — Participação e herança
Map ID: M09.01
Complexity: high
Files:
- `tests/test_sdd_flow_m09_network.py`
...
- [ ] Escrever em `tests/test_sdd_flow_m09_network.py` testes positivos e negativos dos critérios desta tarefa; executar `python3 -m unittest tests.test_sdd_flow_m09_network` e comprovar RED pelo comportamento ausente, nunca por zero testes ou erro de importação.
...
- [ ] Executar `python3 -m unittest tests.test_sdd_flow_m09_network`, confirmar mais de zero testes, resultado sem falhas e validar os links/inputs/outputs dos arquivos listados.
